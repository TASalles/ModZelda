...
public class TrdWalk : MonoBehaviour
{
    ...
    {
        StartCoroutine(Idle());
        if (SceneManager.GetActiveScene().name == "Srine01")
        {
            currentHealth = maxHealth;
            healthBar.SetMaxHealth(maxHealth);
        }
        if (SceneManager.GetActiveScene().name == "Srine02")
        {
            currentHealth = maxHealth;
            healthBar.SetMaxHealth(maxHealth);
        }
        if (SceneManager.GetActiveScene().name == "Srine03")
        {
            currentHealth = maxHealth;
            healthBar.SetMaxHealth(maxHealth);
        }
        if (SceneManager.GetActiveScene().name == "Srine04")
        {
            currentHealth = maxHealth;
            healthBar.SetMaxHealth(maxHealth);
        }
    ...
    private void RestorHealth(int health)
    {
    ...
    {
        if (collision.collider.CompareTag("IA"))
        {
            if(iaWalk.currentState == IAWalk.IaState.Attack)
            TakeDamage(2);
        }
